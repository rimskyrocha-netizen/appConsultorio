...
const Notices: React.FC = () => {
  const [notices, setNotices] = useState<Notice[]>(() => {
    const saved = localStorage.getItem('odontoflow_notices');
  ...
  useEffect(() => {
    localStorage.setItem('odontoflow_notices', JSON.stringify(notices));
  }, [notices]);
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      setNotices(prev => prev.map(n => n.id === editingId ? { ...n, ...formData } : n));
    } else {
  ...
  const toggleStatus = (id: string) => {
    setNotices(prev => prev.map(n => n.id === id ? { ...n, status: n.status === 'PENDENTE' ? 'CONCLUIDO' : 'PENDENTE' } : n));
  };
