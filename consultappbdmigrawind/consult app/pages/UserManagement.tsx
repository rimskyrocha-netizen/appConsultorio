...
const UserManagement: React.FC = () => {
  ...
  // Removido useEffect que sincronizava usuários com localStorage
  ...
  const handleSave = async () => {
    ...
    if (!editingUser && !formData.password) return alert('A senha é obrigatória para novos usuários.');

    const record = {
      ...formData,
      id: editingUser?.id || undefined,
      tenantId: editingUser?.tenantId || 't1',
      lastAccess: editingUser?.lastAccess || 'Nunca',
      // If editing and password is empty, don't change it (controller will handle if we send it null? No, let's keep it safe)
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (editingUser) setUsers(prev => prev.map(u => u.id === editingUser.id ? saved : u));
