...
const Agenda: React.FC<{ patients?: Patient[] }> = ({ patients: propsPatients }) => {
  const location = useLocation();
  const navigate = useNavigate();
  const [viewMode, setViewMode] = useState<'day' | 'week' | 'month' | 'year'>('day');
  
  const [appointments, setAppointments] = useState<Appointment[]>([]);
  const [patients, setPatients] = useState<Patient[]>(propsPatients || []);
  const [dentistList, setDentistList] = useState<any[]>([]);

  useEffect(() => {
    // Carrega usuários para extrair dentistas
    fetch(`${API_BASE_URL}/api/users`)
      .then(res => res.json())
      .then(allUsers => {
        if (Array.isArray(allUsers)) {
          const filtered = allUsers.filter(u => 
            (u.role === 'DENTIST' || u.role === 'ADMIN') && u.status === 'ACTIVE'
          );
          ...
          setDentistList(filtered.map((u, i) => ({
            id: u.id,
            name: u.name,
  ...
  // Removido sync com localStorage pois agora usamos a API
  /*
  useEffect(() => {
  ...
  */
  ...
  // A lista de dentistas agora é gerenciada pelo estado dentistList preenchido via API
