...
const Procedures: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/base-procedures`)
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) setProcedures(data);
      })
      .catch(err => console.error('Erro ao buscar catálogo de procedimentos:', err));
  }, []);

  // Removido useEffect que sincronizava procedimentos com localStorage
  ...
  const handleSave = async () => {
    ...
    // Convert current formData to a format suitable for the API
    const record = {
      ...formData,
      id: editingProcedure?.id || undefined
    };

    try {
      const response = await fetch(`${API_BASE_URL}/api/base-procedures`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        if (editingProcedure) {
          setProcedures(prev => prev.map(p => p.id === editingProcedure.id ? saved : p));
        } else {
      ...
      console.error('Erro ao salvar procedimento no catálogo:', err);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Tem certeza que deseja excluir este procedimento?')) {
      try {
        const response = await fetch(`${API_BASE_URL}/api/base-procedures/${id}`, { method: 'DELETE' });
        if (response.ok) {
