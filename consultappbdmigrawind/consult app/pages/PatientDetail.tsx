...
const PatientDetail: React.FC<PatientDetailProps> = ({ patients, setPatients }) => {
  ...
  // Sincronização dos dados ao mudar de paciente
  useEffect(() => {
    // Busca dentistas para a aba de evoluções
    fetch(`${API_BASE_URL}/api/users`)
      .then(res => res.json())
      .then(data => { if (Array.isArray(data)) setDentists(data.filter(u => u.role === 'DENTIST' || u.role === 'ADMIN')); })
      .catch(err => console.error('Erro ao buscar dentistas:', err));

    if (id) {
      // Carrega evoluções da API
      fetch(`${API_BASE_URL}/api/evolutions/patient/${id}`)
        .then(res => res.json())
        .then(data => {
          if (Array.isArray(data)) setEvolutions(data);
        })
        .catch(err => console.error('Erro ao buscar evoluções:', err));

      // Carrega plano de tratamento da API
      fetch(`${API_BASE_URL}/api/treatments/patient/${id}`)
        .then(res => res.json())
        .then(data => {
      ...
      // Carrega pagamentos da API
      fetch(`${API_BASE_URL}/api/payments/patient/${id}`)
        .then(res => res.json())
        .then(data => {
      ...
      // Carrega anamnese do objeto patient (que vem da API via props)
      const foundPatient = patients.find(p => p.id === id);
      if (foundPatient && foundPatient.medicalHistory) {
        try {
          setAnamnese(JSON.parse(foundPatient.medicalHistory));
        } catch (e) {
          console.error('Erro ao converter anamnese:', e);
        }
      } else {
        setAnamnese({
          mainComplaint: '',
  ...
  // Removido useEffect que sincronizava plano, pagamentos e evoluções com localStorage
  ...
  // Removido useEffect que sincronizava anamnese com localStorage
  ...
  const handleSavePatientEdit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Erro ao salvar no banco de dados: ' + errorText);
      }
    } catch (error) {
      console.error('Erro ao atualizar paciente:', error);
      alert('Erro de conexão com o servidor ao salvar alterações.');
    }
  };
  ...
  const handleSaveAnamnese = async () => {
    try {
      const updatedPatient = { 
        ...patient, 
        medicalHistory: JSON.stringify(anamnese) 
      };
      ...
      if (response.ok) {
        ...
        if (setPatients) setPatients(prev => prev.map(p => p.id === id ? savedPatient : p));
        setAnamneseSaved(true);
        setTimeout(() => setAnamneseSaved(false), 3000);
      } else {
        alert('Erro ao salvar anamnese no servidor.');
      }
    } catch (err) {
      console.error('Erro ao salvar anamnese:', err);
      alert('Erro de conexão ao salvar ficha de anamnese.');
    }
  };
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (editingPlanProc.id) {
          setPlan(prev => prev.map(p => p.id === editingPlanProc.id ? saved : p));
        } else {
          setPlan(prev => [...prev, saved]);
        }
...
Estou ciente de que o sucesso do tratamento depende também da minha colaboração, seguindo as recomendações pós-operatórias e comparecendo às consultas agendadas.`);
  ...
  const handleSaveEvolution = async () => {
    ...
    const record = {
      id: editingEvoId || undefined,
      patientId: id,
      date: editingEvoId 
        ? evolutions.find(e => e.id === editingEvoId)?.date 
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/evolutions`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(record)
      });
      ...
      if (response.ok) {
        const savedEvo = await response.json();
        if (editingEvoId) {
          setEvolutions(prev => prev.map(evo => evo.id === editingEvoId ? savedEvo : evo));
        } else {
          setEvolutions([savedEvo, ...evolutions]);
        }
        setNewEvolutionData({ notes: '', tags: [] });
        setEditingEvoId(null);
        setIsEvolutionModalOpen(false);
      } else {
        alert('Erro ao salvar evolução no banco de dados.');
      }
    } catch (err) {
  ...
  const handleDeleteEvolution = async (evoId: string) => {
    if (!window.confirm('Excluir esta evolução clínica permanentemente?')) return;
    
    try {
      const response = await fetch(`${API_BASE_URL}/api/evolutions/${evoId}`, {
        method: 'DELETE'
      });
  ...
  const handleOdontogramAdd = async (newProc: TreatmentProcedure) => {
    const record = { ...newProc, patientId: id };
    try {
      const response = await fetch(`${API_BASE_URL}/api/treatments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      console.error('Erro no odontograma (add):', err);
    }
  };

  const handleOdontogramUpdate = async (updatedProc: TreatmentProcedure) => {
    const record = { ...updatedProc, patientId: id };
    try {
      const response = await fetch(`${API_BASE_URL}/api/treatments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        const saved = await response.json();
        setPlan(prev => prev.map(p => p.id === updatedProc.id ? saved : p));
      }
    } catch (err) {
      console.error('Erro no odontograma (update):', err);
    }
  };

  const handleOdontogramDelete = async (procId: string) => {
    if (!window.confirm('Excluir este procedimento do plano?')) return;
    try {
      const response = await fetch(`${API_BASE_URL}/api/treatments/${procId}`, {
        method: 'DELETE'
      });
      if (response.ok) {
        setPlan(prev => prev.filter(p => p.id !== procId));
      }
    } catch (err) {
      console.error('Erro no odontograma (delete):', err);
    }
  };

  const updateReceiptProfessional = (name: string) => {
    const matched = dentists.find(u => u.name.toLowerCase().includes(name.toLowerCase()));
    if (matched) {
      if (matched) {
        setReceiptData({
          ...receiptData,
