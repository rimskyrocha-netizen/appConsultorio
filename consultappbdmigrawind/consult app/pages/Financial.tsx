...
import { Patient, TreatmentProcedure, PaymentRecord } from '../types';
...
const Financial: React.FC<{ patients?: Patient[] }> = ({ patients: propsPatients }) => {
  const location = useLocation();
  const navigate = useNavigate();
  
  const [globalTransactions, setGlobalTransactions] = useState<Transaction[]>([]);
  const [allPayments, setAllPayments] = useState<PaymentRecord[]>([]);
  const [allTreatments, setAllTreatments] = useState<TreatmentProcedure[]>([]);

  useEffect(() => {
    // 1. Fetch global transactions
    fetch(`${API_BASE_URL}/api/transactions`)
      .then(res => res.json())
      .then(data => { if (Array.isArray(data)) setGlobalTransactions(data); })
      .catch(err => console.error('Erro ao buscar transações:', err));

    // 2. Fetch all treatments to calculate debts (projected)
    fetch(`${API_BASE_URL}/api/treatments`)
      .then(res => res.json())
      .then(data => { if (Array.isArray(data)) setAllTreatments(data); })
      .catch(err => console.error('Erro ao buscar tratamentos:', err));

    // 3. Fetch all payments from all patients
    fetch(`${API_BASE_URL}/api/payments`)
      .then(res => res.json())
      .then(data => { if (Array.isArray(data)) setAllPayments(data); })
      .catch(err => console.error('Erro ao buscar pagamentos:', err));
  }, []);

  // Removido sync com LocalStorage para transações globais

  // Consolidação de dados de todos os pacientes
  const consolidated = useMemo((): ConsolidatedFinancials => {
    // Busca os pacientes dos props (API) ou fallback para localStorage
    let patients: Patient[] = [];
    if (propsPatients && propsPatients.length > 0) {
    ...
    patients.forEach(p => {
      // Usar os dados já carregados para este paciente
      const payments = allPayments.filter(pay => pay.patientId === p.id);
      const plan = allTreatments.filter(t => t.patientId === p.id);
  ...
  }, [globalTransactions, allPayments, allTreatments, propsPatients]);
  ...
  const handleSaveEntry = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setGlobalTransactions([saved, ...globalTransactions]);
